...
#just a false class to prevent errors
class ServoMotors():
    def __init__(self,pin):
        self.pin=pin
    def angle(self,ang):
        print("pin No "+str(self.pin)+" moved to "+str(ang)+" degrees")
class ServoKit():
    def __init__(self,channel):
        self.servo=[]
        for i in range(channel):
            self.servo.append(ServoMotors(i))
...
class RoboServo ():
    def __init__(self,pinNumber, offset, min_thresh, max_thresh, direc):
        self.Motor_pin=int(pinNumber)
    ...
    def motor_init(self,PCA_pin):
        self.Motor_pin =int( PCA_pin)
  
    def turn_absolute(self, angle=0):
        ...
        kit.servo[self.Motor_pin].angle(FinalAngle)
...
Right_Ankle_Sideways = RoboServo(0, 83, 45, 134, NORMAL)  #positive means bottom of foot faces outside,negative means it faces inside
Right_Ankle_UpDown = RoboServo(0, 90, 45, 180, REVERSE)    #positive means toe points upward, negative means toe points downward
Left_Ankle_Sideways = RoboServo(10, 97, 58, 145, REVERSE)  #positive means bottom of foot faces outside,negative means it faces inside
Left_Ankle_UpDown = RoboServo(0, 90, 0, 135, NORMAL)
...
#************************************************************************************/
  # turns the humaoin from one position to the other but with multiple steps in between
